"""
Modified on https://github.com/sksq96/pytorch-summary for specifying the device.
"""
import os
...
__all__ = ['summary_to_txt', 'summary']


def summary_to_txt(model, config, device, pth):
    def print_fn(line):
        print(line)
        with open(pth, 'a', encoding='utf-8') as fp:
            fp.write(line + "\n")
    if path.exists(pth):
        os.remove(pth)
    summary(model, (3 if config.color else 1, *config.sz_a, *config.train_patch_sz), batch_size=config.train_batch_sz,
            device=device, print_fn=print_fn)


def summary(model, input_size, batch_size=-1, device="cuda", print_fn=print):
    # Modified on TorchSummary
